...
int Snake()
{
	...
	while (!gameOver)
	{
		...
		if ((snake.at(0).at(0) < 0 || snake.at(0).at(0) >= screenWidth || snake.at(0).at(1) < 3 || snake.at(0).at(1) >= screenHeight) || (screen[snake.at(0).at(1) * screenWidth + snake.at(0).at(0)] == L"o"[0]))
		{
			...
			leaderBoard.open("snakeHighScore.txt");

			if (leaderBoard.peek() != ifstream::traits_type::eof())
				getline(leaderBoard, oldHighScore);

			leaderBoard.close();

			if (score > stoi(oldHighScore))
			{
				wsprintf(&screen[10 * screenWidth + 33], L"NEW HIGH SCORE");

				leaderBoard.open("snakeHighScore.txt", fstream::out);
				...
				wsprintf(&screen[10 * screenWidth + 28], L"CURRENT HIGH SCORE: %d", stoi(oldHighScore));
		}
		else
		{
		...
		while (gameOver)
		{
			...
			newGame = (0x8000 & GetAsyncKeyState((unsigned char)("\x0D"[0]))) != 0;
			quit = (0x8000 & GetAsyncKeyState((unsigned char)("\x1B"[0]))) != 0;
